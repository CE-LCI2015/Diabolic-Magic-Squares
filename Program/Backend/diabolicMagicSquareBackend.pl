...
generateAll(L):- createAllBases(A), rotateAll(A,B), rotateAllAtCenter(B,C), rotateAllRows(C,E),  globalConvolution(E,F), reflectAll(F,G), zeroWithTwoColsAll(G,H), oneWithThreeColsAll(H,I), zeroWithTwoRowsAll(I,J), oneWithThreeRowsAll(J,K), lastCheck(K,L).
